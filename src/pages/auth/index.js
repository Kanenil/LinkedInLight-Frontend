...
const Auth = () => {
	...
	useEffect(() => {
		if (!isMobile) navigator(routes.signIn)
	}, [isMobile])
		...
		<React.Fragment>
			...
			<main className='flex-grow flex flex-col px-4 py-5'>
				...
				<div className='flex flex-col'>
					...
					<h3 className='font-jost font-medium text-center'>
						{t("auth.illustrationText")}
					</h3>
				</div>

				<div className='flex flex-col gap-4 mt-10'>
					<h3
						className='font-jost text-[#7D7D7D] text-sm [&>strong]:font-medium mb-4'
						dangerouslySetInnerHTML={{ __html: t("auth.confirmText") }}
					/>
