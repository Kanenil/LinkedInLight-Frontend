...
const ConfirmEmail = () => {
	...
	const onChange = e => {
		...
		if (value.length === 0) setCode("______".slice(0, 6))
	}
						...
						>
							<div className='flex h-full items-center'>
								<form onSubmit={onSubmit} className='mx-auto my-12'>
									...
									<h1 className='text-[#2D2A33] font-bold text-2xl text-center my-7'>
										{t("auth.confirmEmail")}
									</h1>

									<h3 className='text-lg text-[#2D2A33]'>
										{t("auth.enterCode")}{" "}
									...
									<h3 className='text-sm text-[#2D2A33] text-center mt-2.5'>
										{t("auth.noCode")}
										<button
											onClick={reSend}
											type='button'
