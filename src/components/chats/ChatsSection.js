...
const ChatsSection = ({ getParticipant, selectedChat, setSelectedChat }) => {
	...
	useEffect(() => {
		...
		if (searchParams.has("createIfNotExist")) {
			ChatService.startChat(searchParams.get("createIfNotExist")).then(
				({ data }) => {
					setSelectedChat(data)
					searchParams.set("chat", data.id)
					searchParams.delete("createIfNotExist")
