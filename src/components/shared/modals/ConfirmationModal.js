...
const ConfirmationModal = ({
	...
	const closeModal = () => {
		setModalState({
			...modalState,
			...(modalState.hasUserInformationChanged
				? { isClosing: true }
				: {
						isModalClosed: true,
						hasUserInformationChanged: false,
		...
		if (!modalState.hasUserInformationChanged && !onCloseCallback) navigator(-1)
	...
	return (
		<ConditionalWrapper condition={isOpen}>
			<Modal
				isOpen={isOpen}
				isRounded={!isMobile}
				closeModal={modalState.isModalClosed}
				hideOnClose={false}
				onClose={closeModal}
				position={position}
			>
				...
				<Modal
					childModal={true}
					isOpen={modalState.isClosing}
					onClose={onCloseConfirm}
					position='mt-24 mx-auto'
				>
