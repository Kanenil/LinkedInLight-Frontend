            ...
            <Show>
                <Show.When isTrue={!!values.image}>
                    <div className="inline-flex gap-3 ml-auto">
                        <label className="cursor-pointer bg-[#C6CCDA] rounded-lg p-1" htmlFor="image">
                            <PencilIcon className="text-white w-5 h-5"/>
                            <input id="image" onChange={onImageChange} className="hidden" type="file"
                                   accept="image/png, image/jpg, image/jpeg"/>
                        </label>

                        <button
                            className="bg-[#C6CCDA] rounded-lg p-1"
                            onClick={() => setValues(async (prev) => await ({...prev, image: ""}))}
                        >
                            <XMarkIcon className="text-white w-5 h-5 stroke-2"/>
                        </button>
                    </div>
                    <div
                        className='flex items-center border-[0.5px] border-[#7D88A4]/50 rounded-lg overflow-hidden my-auto w-full h-fit bg-white'>
                        <img
                            className='object-contain'
                            src={values.image}
                            alt='company-logo'
