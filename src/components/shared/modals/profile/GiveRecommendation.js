...
const GiveRecommendation = ({ onClose, onSave, onChange, id }) => {
	...
	const onSubmitFormik = async values => {
		RecommendedProfileService.giveRecommendation(values.content, id).then(
			() => {
	...
	useEffect(() => {
		if (id) {
			RecommendedProfileService.recommendationById(id).then(
				({ data: recommendation }) => {
					if (!recommendation) return onClose()

					if (recommendation.status !== "Pending") return onClose()

					const {
						requester: { firstName, lastName, image, headline },
					} = recommendation
	...
	return (
		<EditModalForm
			saveTitle={t("profile.modal.request.send")}
			header={t("profile.modal.giveRecommendation.writeFor", {
		...
		>
			<h3 className='mt-2 font-jost text-[#2D2A33] font-light text-sm'>
				{t("validation.requiredField")}
			</h3>

			<div className='mt-3 flex flex-col'>
				<h1 className='font-jost font-light text-[#2D2A33] text-lg'>
					{t("profile.modal.giveRecommendation.willShownIn", {
				...
				<div className='flex flex-row gap-3 mt-3'>
					<div className='overflow-hidden h-10 w-10 my-auto bg-white rounded-full border-[3px] border-[#FFFFFF] bg-[#EAEAEA]'>
						<img
							...
							src={
								values.image
									? APP_ENV.UPLOADS_URL + "/" + values.image
									: defaultImage
							}
							alt=''
						/>
			...
			<ModalTextareaFormGroup
				title={t("profile.modal.giveRecommendation.title")}
				name='content'
