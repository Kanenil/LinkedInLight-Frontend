...
const EditContactInformation = ({ onClose, onSave, onChange }) => {
	...
	const { values, onChangeInput, onSubmit, setErrors, setValues } = useForm(
		initialValues,
		onChange,
	)
	...
	const onSaveClick = async () => {
		...
		AdditionalProfileService.updateContactInformation(model).then(() => {
	...
	return (
		...
		>
			<div className='flex flex-col gap-2 mt-2.5'>
				...
				<Link
					onClick={onClose}
					to={`${APP_ENV.FRONTEND_URL}/j4y/${values.profileUrl}`}
