...
const VolunteerExperienceSection = ({ user }) => {
	...
	const { data, isLoading } = useQuery({
		queryFn: ({ queryKey }) =>
			AdditionalProfileService.getVolunteerExperiencesByProfileUrl(queryKey[1]),
				...
				<div className='px-6 md:px-10'>
					<div className='flex flex-row items-center gap-[20px]'>
						<h1 className='font-jost font-medium text-2xl text-[#2D2A33]'>
							{t("volunteerExperiencesSection.title")}
						</h1>
					...
					<div className='flex flex-col mt-2.5 gap-[25px] py-[5px]'>
						{data.map((volunteerExperience, index) => (
							<VolunteerExperienceItem
								{...volunteerExperience}
								key={`volunteerExperiences-${volunteerExperience.name}-${index}`}
							/>
						))}
					</div>
				</div>
