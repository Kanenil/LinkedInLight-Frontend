...
const VolunteerExperienceItem = ({editPath, isShowMore = true, startDate, endDate, currentlyVolunteering, organization, description, role, cause}) => {
    ...
    const period = (
        `${getShortMonth(start.getMonth())} ${start.getFullYear()} - ${currentlyVolunteering ? "Present" : `${getShortMonth(end.getMonth())} ${end.getFullYear()}`}`
    )
    ...
    return (
        <div className="mt-2.5 py-2.5 flex flex-row gap-5">
            <div className="pb-[5px] font-jost">
                ...
                <h3 className="font-light font-normal text-[#2D2A33] text-sm">{ organization }</h3>
                <h3 className="font-light font-normal text-[#2D2A33] text-sm">{ cause }</h3>
